...
const search = async (req, res) => {
  ...
  const sql = `
  SELECT U.id, U.name, 
    CASE WHEN F.friendId IS NOT NULL THEN 1 ELSE 0 END as connection
  FROM Users U LEFT JOIN Friends F ON U.id = F.friendId AND F.userId = ${userId}
  WHERE U.name LIKE '${query}%'
  LIMIT 20;
  `

  const old = `SELECT id, name, id in (SELECT friendId from Friends where userId = ${userId}) as connection from Users where name LIKE '${query}%' LIMIT 20;`
  
  db.all(sql).then((results) => {
    res.statusCode = 200;
    res.json({
      success: true,
      users: results
